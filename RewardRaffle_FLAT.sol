...
contract RewardRaffle is Ownable {
    ...
    address FTSOAddr = 0xA1a9B8aB5BB798EeE536A23669AD744DCF8537a3;
    IFtso ftsoContract = IFtso(FTSOAddr);

    address RManAddr = 0xc5738334b972745067fFa666040fdeADc66Cb925;
    IFtsoRewardManager rmanContract = IFtsoRewardManager(RManAddr);

    address pinkAddr = 0xAD8fC70Af4d8DEC3E80F203Eade3cCeC3dbd5114;
    IPinkRaffle pinkContract = IPinkRaffle(pinkAddr);

    function setWNatAddr(address _WNat) public onlyOwner{
       WNatAddr = _WNat;
    }

    function setFTSOAddr(address _FTSO) public onlyOwner{
       FTSOAddr = _FTSO;
    }
    ...
    function setpinkAddr(address _pink) public onlyOwner{
       RManAddr = _pink;
    }
    ...
    function collect() public payable {
        require (!lock);
        require(addressToAllowance[msg.sender] > 0, "Nothing to collect.");
        lock = true;
        (bool success, ) = payable(msg.sender).call{value: addressToAllowance[msg.sender]}("");
        require(success);
        addressToAllowance[msg.sender] = 0;
        lock = false;
    }

    function addAllowance(address _winner, uint256 _amount) private {
        addressToAllowance[_winner] += _amount;
    }

    //manual winner designation if automatic calling of winner fails
    ...
    //claim reward and draw winner
    function claim() public payable onlyOwner {
        require(rmanContract.getEpochsWithUnclaimedRewards(address(this)).length > 0);
        require(!lock);
        lock = true;
        uint256 winnings = rmanContract.claimReward(payable(address(this)), rmanContract.getEpochsWithUnclaimedRewards(address(this)));
        internalWinner(generateRandom(), winnings);
        lastReward = winnings;
        lock = false;
    }
    ...
    function generateRandom() public returns(uint256) {
        uint256 _epoch = ftsoContract.getCurrentEpochId();
        uint256 _random = ftsoContract.getCurrentRandom();
        uint256 _price;
        (_price, ) = ftsoContract.getCurrentPrice();

        _num = _random.div(_price.mul(_epoch)).mod(pinkContract.totalSupply()) + 1;
        _ep = _epoch;
        _pr = _price;
        _ra = _random;
    ...
    function seeNum() public view returns (uint256 ){
        return _num;
    }

    function seeEpoch() public view returns (uint256 ){
        return _ep;
    }
    ...
    function seeRandom() public view returns (uint256 ){
        return _ra;
    }

    function seeLock() public view returns (bool ){
        return lock;
    }

    function seeWinners() public view returns (uint256[] memory){
